...
logging.info("logging is correct")
...
for ex in dataset.take(3):
  logging.info(ex)
logging.info("_________need time_________")
loss, log_probs, grads = fwd_bwd(ex[0], ex[1])
logging.info('Loss: ', loss.numpy())

ckpt_path = 'bigbird/ckpt/pretrain/bigbr_base/model.ckpt-0'
ckpt_reader = tf.compat.v1.train.NewCheckpointReader(ckpt_path)
logging.info("_________success restore_________")
...
for i, ex in enumerate(tqdm(dataset.take(FLAGS.num_train_steps), position=0)):
  ...
  if i% 200 == 0:
    logging.info('Loss = {}  Accuracy = {}'.format(train_loss.result().numpy(), train_accuracy.result().numpy()))
...
logging.info('Loss = {}  Accuracy = {}'.format(eval_loss.result().numpy(), eval_accuracy.result().numpy()))
